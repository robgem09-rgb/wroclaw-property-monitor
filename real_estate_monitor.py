...
class RealEstateMonitor:
    ...
    def scrape_olx(self) -> List[Dict]:
        """Zbiera oferty z OLX - wersja mniej wybredna"""
        properties = []
        criteria = self.config.get('criteria', {
            'min_price': 0, 'max_price': 99999999, 
            'min_area': 0, 'max_area': 999
        ...
        try:
            # Używamy sortowania po najnowszych
            base_url = "https://www.olx.pl/nieruchomosci/mieszkania/sprzedaz/wroclaw/?search[order]=created_at:desc"
            response = self.session.get(base_url, timeout=15)
            
            if response.status_code != 200:
                print(f"  ✗ OLX Status: {response.status_code}", flush=True)
                return []

            soup = BeautifulSoup(response.content, 'html.parser')
            listings = soup.find_all('div', {'data-testid': 'ad-card'})
            
            print(f"  DEBUG: Znaleziono {len(listings)} ogłoszeń na stronie", flush=True)
            ...
            for listing in listings:
                try:
                    ...
                    if 'promoted' in url: continue # Omijamy sponsorowane, by nie dublować
                    ...
                    # Próba wyciągnięcia metrażu z tytułu lub opisu na karcie
                    area = self.extract_area(title)
                    if not area:
                        # Szukamy tekstu z metrażem w całym bloku ogłoszenia
                        area = self.extract_area(listing.get_text(" "))

                    # LOGIKA FILTROWANIA (Mniej restrykcyjna)
                    # 1. Sprawdzamy cenę
                    price_ok = (criteria['min_price'] <= price <= criteria['max_price'])
                    
                    # 2. Sprawdzamy metraż (jeśli area jest 0 lub None, też przepuszczamy!)
                    area_ok = (area is None or area == 0 or (criteria['min_area'] <= area <= criteria['max_area']))
                    ...
                    if price_ok and area_ok:
                        ...
                        # Logujemy dlaczego odrzucono (tylko w trybie debug)
                        if not price_ok:
                            print(f"    - Odrzucono (cena: {price})", flush=True)
                        elif not area_ok:
                            print(f"    - Odrzucono (metraż: {area})", flush=True)

                except Exception as e:
                    continue

            print(f"  ✓ OLX: Dodano {len(properties)} ofert do listy", flush=True)
